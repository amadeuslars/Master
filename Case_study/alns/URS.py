...
# (Scoring and Weight Decay parameters have been removed for Uniform Random Selection)

# ---------------------------------------------------------
#  ACTION SPACE SETUP (Composite Actions)
# ---------------------------------------------------------
...
# We don't strictly need NUM_ACTIONS anymore since we just pick from the list directly,
# but it's good for keeping track of the size of the action space.
...
def run_alns_uniform():
    ...
    for i in range(MAX_ITERATIONS):
        ...
        # UNIFORM RANDOM SELECTION: Pick a random action from the list
        d_op, r_op, label = random.choice(actions)
        ...
        # 3. ACCEPTANCE (No Scoring/Rewards required)
        accepted = False
        ...
        # (Weight update logic completely removed)
